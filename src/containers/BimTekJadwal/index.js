        ...
        <Row className="bimtek-filter mb-3">
          <Col xs={4}>
            <Form.Select>
              ...
              {filterCategory.map((category, key) => (<option key={key}>{category}</option>))}
            </Form.Select>
          </Col>
          <Col xs={4}>
            <Form.Select>
              <option>Pilih Kota Pelaksanaan</option>
              {filterCity.map((city, key) => (<option key={key}>{city}</option>))}
            </Form.Select>
          </Col>
          <Col xs={2}>
            <Form.Select>
              {filterYear.map((year, key) => (<option key={key}>{year}</option>))}
            </Form.Select>
          </Col>
        </Row>
